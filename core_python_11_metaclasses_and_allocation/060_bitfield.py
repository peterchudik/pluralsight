...
class BitFieldBase:
    ...
    def _validate_arg_names(self, kwargs):
        ...
        if len(mismatched_args) != 0:
            raise TypeError(
                "{}.__init__() got unexpected argument{}: {}".format(
                    type(self).__name__,
    ...
    #                 f"range {min_value!r}-{max_value!r} for a {width!r} bit fields"
    #             )
...
class BitFieldMeta(type):
    ...
    def __new__(mcs, name, bases, namespace, **kwargs):
        ...
        # reads class anotations (dict) and save it to private attribute
        ...
        for field_name, width in field_widths.items():
            ...
            if field_name.startswith("_"):
                raise TypeError(
                    f"{name} field {field_name!r} begins with underscore"
                )
        ...
        # pre-pend to all base classes
        # this way __init__ of BitFieldBase will defined in class
    ...
    def __set__(self, instance, value):
        ...
        if not (min_value <= value <= max_value):
            raise ValueError(
                ...
                f"got value {value!r} which is out of "
                f"range {min_value!r}-{max_value!r} for a {self._width!r} bit fields"
            )
